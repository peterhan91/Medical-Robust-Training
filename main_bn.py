        ...
        for epoch in range(1, args.max_epoch+1):
            for data, label in tr_loader:
                ...
                if adv_train:
                    ...
                    output = model(data, [0])
                loss = F.binary_cross_entropy(F.sigmoid(output), label)
                loss_ = F.binary_cross_entropy(F.sigmoid(output_), label)
                loss_t = loss + loss_
                opt.zero_grad()
                loss_t.backward()
                opt.step()
    ...
    def test(self, model, loader, adv_test=False, 
        ...
        with torch.no_grad():
            for data, label in loader:
                ...
                model.eval()
                output = model(data, [0])
                std_loss = F.binary_cross_entropy(F.sigmoid(output), label)
                pred = F.sigmoid(output)
                out = (pred > t).float()
                ...
                if adv_test:
                    ...
                    model.eval()
                    adv_output = model(adv_data, [1])
                    adv_loss = F.binary_cross_entropy(F.sigmoid(adv_output), label)
                    adv_pred = F.sigmoid(adv_output)
