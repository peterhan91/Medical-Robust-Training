...
from model.model_linear import SimpleModel
    ...
    def train(self, model, tr_loader, va_loader=None, adv_train=False):
        ...
        for epoch in range(1, args.max_epoch+1):
            for data, label in tr_loader:
                ...
                if adv_train:
                    adv_data = self.attack.perturb(data, label, 'mean', True, True, True)
                    model.train()
                ...
                if _iter % args.n_eval_step == 0:
                    if adv_train:
                        ...
                        pred = torch.max(output_, dim=1)[1]
                        adv_acc = evaluate(pred.cpu().numpy(), label.cpu().numpy()) * 100
                    else:
                        adv_data = self.attack.perturb(data, label, 'mean', False, True, True)
                        with torch.no_grad():
                            model.eval()
...
def main(args):
    ...
    if args.todo == 'train':
        ...
        va_dataset = tv.datasets.MNIST(args.data_root, 
                                       train=False, 
                                       transform=tv.transforms.ToTensor(), 
                                       download=True)
        te_loader = DataLoader(va_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4)
