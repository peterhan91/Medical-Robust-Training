        ...
        for epoch in range(1, args.max_epoch+1):
            for data, label in tr_loader:
                ...
                if adv_train:
                    ...
                    adv_data = self.attack.perturb(data, label, 'mean', True, False, True)
                    model.train()
                    output = model(data) 
                else:
                    model.train()
                ...
                if _iter % args.n_eval_step == 0:
                    if adv_train:
                        ...
                        adv_data = self.attack.perturb(data, label, 'mean', False, False, True)
                        with torch.no_grad():
                            model.eval()
        ...
        with torch.no_grad():
            for data, label in loader:
                ...
                if adv_test:
                    ...
                    adv_data = self.attack.perturb(data, pred, 'mean', False, False, True)
                    model.eval()
                    adv_output = model(adv_data)
